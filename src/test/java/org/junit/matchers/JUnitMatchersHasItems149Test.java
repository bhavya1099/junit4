...
public class JUnitMatchersHasItems149Test {
	@Test
	public void matchCollectionElementsWithMatchers() {
		// Arrange
		final Iterable<Integer> collection = Arrays.asList(1, 2, 3, 4, 5);
		final Matcher<? super Integer> matcherOne = is(1);
		final Matcher<? super Integer> matcherTwo = is(2);
		// Act
		final Matcher<Iterable<Integer>> result = JUnitMatchers.hasItems(matcherOne, matcherTwo);
        
		// Assert
		assertThat(collection, is(result));
	}
    
	@Test
	public void notMatchInEmptyCollection() {
		// Arrange
		final Iterable<Integer> emptyCollection = Collections.emptyList();
		final Matcher<? super Integer> matcher = is(1);
		// Act
		final Matcher<Iterable<Integer>> result = JUnitMatchers.hasItems(matcher);
        
		// Assert
		assertThat(emptyCollection, is(result));
	}
	...
	public void failToMatchUsingExactMatchers() {
		// Arrange
		final Iterable<Integer> collection = Arrays.asList(1, 2, 3, 4, 5);
		final Matcher<? super Integer> matcherOne = is(6);
		final Matcher<? super Integer> matcherTwo = is(7);
		// Act
		final Matcher<Iterable<Integer>> result = JUnitMatchers.hasItems(matcherOne, matcherTwo);
