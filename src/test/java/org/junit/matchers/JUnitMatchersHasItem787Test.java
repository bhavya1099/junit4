...
public class JUnitMatchersHasItem787Test {

	@Test
	public void testHasItemWhenItemExists() {
		Iterable<Integer> integers = Arrays.asList(1, 2, 3, 4, 5);
		Matcher<Iterable<? super Integer>> matcher = CoreMatchers.hasItem(3);
		assertThat("Has item check failed when item is present", matcher.matches(integers), is(true));
	}

	@Test
	public void testHasItemWhenItemDoesNotExist() {
		Iterable<Integer> integers = Arrays.asList(1, 2, 3, 4, 5);
		Matcher<Iterable<? super Integer>> matcher = CoreMatchers.hasItem(7);
		assertThat("Has item check passed when item is absent", matcher.matches(integers), is(false));
	}

	@Test
	public void testHasItemWithEmptyCollection() {
		Iterable<Integer> integers = Collections.emptyList();
		Matcher<Iterable<? super Integer>> matcher = CoreMatchers.hasItem(1);
		assertThat("Has item check passed with empty collection", matcher.matches(integers), is(false));
	}
