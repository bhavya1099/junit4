...
public class JUnitMatchersHasItems506Test {

	@Test
	public void matchCollectionElementsWithMatchers() {
		List<Integer> testCollection = new ArrayList<>(Arrays.asList(1, 2, 3, 4, 5));
		Matcher<Iterable<? super Integer>> matchers = CoreMatchers.hasItems(1, 2);
		boolean matches = matchers.matches(testCollection);
		assert matches;
	}
	...
	public void failToMatchUsingExactMatchers() {
		List<Integer> testCollection = new ArrayList<>(Arrays.asList(1, 2, 3, 4, 5));
		Matcher<Iterable<? super Integer>> matchers = CoreMatchers.hasItems(6, 7);
		boolean matches = matchers.matches(testCollection);
		assertFalse(matches);
	}
