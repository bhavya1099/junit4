...
@Category(Categories.everyItem.class)
public class JUnitMatchersEveryItemTest {

	@Rule
	public final ExpectedException expectedException = ExpectedException.none();

	@Test
	public void everyItemReturnsTrueForMatchingElements() {
		Matcher<Integer> greaterThanTen = CoreMatchers.is(CoreMatchers.greaterThan(10));
		Iterable<Integer> numbers = Arrays.asList(11, 12, 13, 14);
		boolean matches = everyItem(greaterThanTen).matches(numbers);
		assertTrue(matches);
	}

	@Test
	public void everyItemReturnsFalseForNonMatchingElements() {
		Matcher<Integer> greaterThanTen = CoreMatchers.is(CoreMatchers.greaterThan(10));
		Iterable<Integer> numbers = Arrays.asList(9, 10, 11, 12);
		boolean matches = everyItem(greaterThanTen).matches(numbers);
		assertFalse(matches);
	}

	@Test
	public void everyItemHandlesEmptyCollection() {
		Matcher<Integer> greaterThanTen = CoreMatchers.is(CoreMatchers.greaterThan(10));
		Iterable<Integer> numbers = Collections.emptyList();
		boolean matches = everyItem(greaterThanTen).matches(numbers);
		assertTrue(matches);
	}

	@Test
	public void everyItemHandlesNullCollection() {
		Matcher<Integer> greaterThanTen = CoreMatchers.is(CoreMatchers.greaterThan(10));
		expectedException.expect(NullPointerException.class);
		everyItem(greaterThanTen).matches(null);
	}
