...
public class JUnitMatchersHasItem805Test {
    @Test
    public void testHasItemWhenItemExists() {
        List<String> list = Arrays.asList("a", "b", "c");
        Matcher<String> matcher = CoreMatchers.is("a");
        Assert.assertTrue("Expected item to be found in list", CoreMatchers.hasItem(matcher).matches(list));
    }
    @Test
    public void testHasItemWhenItemDoesNotExist() {
        List<String> list = Arrays.asList("a", "b", "c");
        Matcher<String> matcher = CoreMatchers.is("d");
        Assert.assertFalse("Expected item to not be found in list", CoreMatchers.hasItem(matcher).matches(list));
    }
    @Test
    public void testHasItemWithEmptyCollection() {
        List<String> list = new ArrayList<String>();
        Matcher<String> matcher = CoreMatchers.is("a");
        Assert.assertFalse("Expected item to not be found in empty list", CoreMatchers.hasItem(matcher).matches(list));
    }
    @Test(expected = NullPointerException.class)
    public void testHasItemWithNullCollection() {
        Matcher<String> matcher = CoreMatchers.is("a");
        CoreMatchers.hasItem(matcher).matches(null);
    }
}