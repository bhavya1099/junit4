...
public class JUnitMatchersHasItems723Test {

	@Test
	public void matchCollectionElementsWithMatchers() {
		List<Integer> numbers = Arrays.asList(1, 2, 3, 4, 5);
		Matcher<Iterable<Integer>> oneMatcher = CoreMatchers.hasItem(1);
		Matcher<Iterable<Integer>> twoMatcher = CoreMatchers.hasItem(2);
		Matcher<Iterable<Integer>> finalMatcher = CoreMatchers.hasItems(oneMatcher, twoMatcher);
		assertThat(numbers, finalMatcher);
	}
	...
	public void notMatchInEmptyCollection() {
		...
		Matcher<Iterable<Integer>> twoMatcher = CoreMatchers.hasItem(2);
		Matcher<Iterable<Integer>> finalMatcher = CoreMatchers.hasItems(oneMatcher, twoMatcher);
		assertThat(emptyList, CoreMatchers.not(finalMatcher));
	}

	@Test
	public void failToMatchUsingExactMatchers() {
		List<Integer> numbers = Arrays.asList(1, 2, 3, 4, 5);
		Matcher<Iterable<Integer>> sixMatcher = CoreMatchers.hasItem(6);
		Matcher<Iterable<Integer>> sevenMatcher = CoreMatchers.hasItem(7);
		Matcher<Iterable<Integer>> finalMatcher = CoreMatchers.hasItems(sixMatcher, sevenMatcher);
		assertThat(numbers, CoreMatchers.not(finalMatcher));
	}
