...
public class MaxCoreRunTest {
	...
	public void setUp() throws IOException {
		// TODO: Change the path as per your local setup
		File storedResults = new File("test-results");
		if (!storedResults.exists()) {
			storedResults.createNewFile();
		}
	...
	public void ensureValidTestClassRunsSuccessfully() {
		// You can replace "ValidTestClass.class" with any valid JUnit TestCase class.
		Result result = maxCore.run(ValidTestClass.class);
	...
	public void handleJUnit3TestClassSuccessfully() {
		// You can replace "JUnit3TestClass.class" with any JUnit3 TestCase class.
		Result result = maxCore.run(JUnit3TestClass.class);
		assertFalse(result.wasSuccessful());
		assertFalse(result.getFailures().isEmpty());
		for (Failure failure : result.getFailures()) {
			assertTrue(
					failure.getDescription().getDisplayName().startsWith(MaxCore.MALFORMED_JUNIT_3_TEST_CLASS_PREFIX));
		}
	}

	@Test
	public void handleMalformedTestClass() {
		Result result = maxCore.run(MalformedTestClass.class);
		assertFalse(result.wasSuccessful());
		assertFalse(result.getFailures().isEmpty());
		for (Failure failure : result.getFailures()) {
			assertTrue(
	...
	public void verifyMaxHistoryUpdateAfterRun() {
		MaxHistory historyBefore = maxCore.getHistory();
		// You can replace "ValidTestClass.class" with any valid JUnit TestCase class.
		maxCore.run(ValidTestClass.class);
		MaxHistory historyAfter = maxCore.getHistory();
		assertNotEquals(historyBefore, historyAfter);
	}
