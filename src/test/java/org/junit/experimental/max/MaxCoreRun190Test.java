...
class MaxCoreRun190Test {
	...
	void setup() {
		...
		maxCore = new MaxCore(new File("storedResults"));
		maxCore.history = history;
	}

	@Test
	void testRunWithValidInput() {
		Request sortedRequest = Request.classes(String.class);
		RunListener listener = history.listener();
		when(history.listener()).thenReturn(listener);
		when(maxCore.sortRequest(request)).thenReturn(sortedRequest);
		Result result = new Result();
		when(core.run(isA(Runner.class))).thenReturn(result);

		Result actualResult = maxCore.run(request, core);

		verify(core, times(1)).addListener(listener);
		verify(core, times(1)).run(sortedRequest.getRunner());
    ...
    void testRunWhenSortingRequestFails() {
      when(maxCore.sortRequest(request)).thenThrow(new RuntimeException());
	...
	void testRunWhenListenerAttachmentFails() {
		doThrow(new RuntimeException()).when(core).addListener(any());
